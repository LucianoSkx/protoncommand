...
func commands() []Command {
	return []Command{
		...
		},
		{
			...
			Description: Localized{
				PT: "Usa o fork vkd3d-low-latency nos jogos Direct3D 12: o equivalente do dxvk-low-latency para D3D12.\nQuando o pacing liga: só se o jogo usar OU a API Reflex OU swapchains DXGI aguardáveis (waitable). O mantenedor estima que cerca de 20-30% dos títulos DX12 usem swapchains aguardáveis (\"my guess would be 20-30%?\"), então confirme com PROTON_LOG=1 procurando \"waitable dxgi swapchain present percentage\" no log do proton (perto de 100% = ativo; 0% ou ausente = o jogo não usa). Se o jogo tiver ambos, o Reflex tem prioridade.\nJogos verificados pelo mantenedor (discussion #2 do projeto): Dead Space Remake, Devil May Cry 5, Forza Horizon 4, Monster Hunter Rise, Overwatch (também tem Reflex), Resident Evil 2, Resident Evil 7 Biohazard, Tom Clancy\"s The Division 2, Street Fighter 6 e Warframe. Witchfire aparece em comentário, com 99.9% de swapchain aguardável, mas não entrou na lista de verificados.\nDois avisos: em jogos Unreal Engine 4 com r.OneFrameThreadLag=1 (o padrão) as swapchains aguardáveis causam queda forte de desempenho — solte um limite de fps (VKD3D_FRAME_RATE) para evitar, mas ele é integrado à lógica de frame pacing do fork, então cuidado para não somar um segundo limitador; e Anti-Lag 2 não é suportado aqui.\nEm GPU AMD o caminho do Reflex funciona por spoofing, mas isso quebra o upgrade FSR4 — ver a entrada de Reflex do low_latency_layer.\nPara jogos Direct3D 8/9/10/11 use PROTON_DXVK_LOWLATENCY.",
				EN: "Uses the vkd3d-low-latency fork in Direct3D 12 games: the D3D12 equivalent of dxvk-low-latency.\nWhen the pacing kicks in: only if the game uses EITHER the Reflex API OR waitable DXGI swapchains. The maintainer estimates that around 20-30% of DX12 titles use waitable swapchains (\"my guess would be 20-30%?\"), so verify with PROTON_LOG=1 and look for \"waitable dxgi swapchain present percentage\" in the proton log (near 100% = active; 0% or missing = the game doesn't use it). If a game has both, Reflex wins.\nVerified by the maintainer (project discussion #2): Dead Space Remake, Devil May Cry 5, Forza Horizon 4, Monster Hunter Rise, Overwatch (also has Reflex), Resident Evil 2, Resident Evil 7 Biohazard, Tom Clancy\"s The Division 2, Street Fighter 6 and Warframe. Witchfire shows up in a comment, at 99.9% waitable swapchains, but did not make the verified list.\nTwo warnings: in Unreal Engine 4 games with r.OneFrameThreadLag=1 (the default) waitable swapchains cause a heavy performance drop — set an fps limit (VKD3D_FRAME_RATE) to avoid it, but that limit is integrated into the fork's frame pacing logic, so be careful not to stack a second limiter; and Anti-Lag 2 is not supported here.\nOn AMD GPUs the Reflex path works through spoofing, but that breaks the FSR4 upgrade — see the low_latency_layer Reflex entry.\nFor Direct3D 8/9/10/11 games use PROTON_DXVK_LOWLATENCY.",
			},
		},
		{
		...
		},
		{
			...
			Description: Localized{
				PT: "Faz o Wine ver só um conjunto de núcleos em vez de todos, o que resolve disputa de agendamento: o jogo fica preso a um subconjunto e sobra núcleo livre para o resto do sistema. O script do Proton repassa esta variável direto para WINE_CPU_TOPOLOGY.\nFormato: nCPUs:lista. A lista pode se repetir até três vezes para dar grupos diferentes a kernel e usuário (n:nKernel:nUser).\nO exemplo acima = 6 CPUs nos núcleos 2 a 7, ou seja, os núcleos 0 e 1 ficam de fora — que é a parte interessante: se você isolou 0 e 1 no kernel para receber IRQ e trabalho em segundo plano (isolcpus com managed_irq, nohz_full, e a afinidade de IRQ em /proc/irq/*/smp_affinity_list), é exatamente eles que você deixa de fora daqui para o jogo não disputá-los.\nAtenção: número menor que o real de núcleos causa stutter, não ganho. Comece pelo total menos os que reservou, não por menos que isso.\nO Proton já aplica limite automático em 20 jogos via default_cpu_limit (Far Cry 2 e 4, The Witcher 2, Space Marine, Prototype, DiRT, DIRT 5 e outros), e nesses títulos essa variável herda o valor do Proton.\nComo verificar: com PROTON_LOG=1, a linha de topologia aparece no log. E ajuste os números antes de colar, senão o jogo roda em um subconjunto que você não escolheu.",
				EN: "Makes Wine see only a subset of cores instead of all of them, which settles scheduling contention: the game is pinned to a subset and cores are left free for the rest of the system. The Proton script passes this variable straight through to WINE_CPU_TOPOLOGY.\nFormat: nCPUs:list. The list can be repeated up to three times to give different groups to kernel and user (n:nKernel:nUser).\nThe example above = 6 CPUs on cores 2 through 7, i.e. cores 0 and 1 stay out — which is the interesting part: if you isolated 0 and 1 in the kernel for IRQs and background work (isolcpus with managed_irq, nohz_full, and IRQ affinity in /proc/irq/*/smp_affinity_list), those are exactly the ones to leave out here so the game doesn't compete for them.\nWatch out: a number lower than your real core count causes stutter, not gains. Start with the total minus the ones you reserved, not less than that.\nProton already applies an automatic limit on 20 games via default_cpu_limit (Far Cry 2 and 4, The Witcher 2, Space Marine, Prototype, DiRT, DIRT 5 and others), and on those titles this variable inherits Proton's value.\nHow to verify: with PROTON_LOG=1, the topology line shows up in the log. And adjust the numbers before pasting, or the game will run on a subset you didn't pick.",
			},
		},
		{
		...
		},
		{
			...
			Description: Localized{
				PT: "Fornece uma substituta da interface Steam Input baseada em XInput, que alguns jogos exigem. Resolve controle fora do Steam e contorna a troca de perfil do Steam Input indisponível no Wine-Wayland.\nQuando fica inativa: se o Steam Input nativo estiver ativo na sessão, o controle e os mapeamentos nativos mandam.\nPor controlador: para controle Sony, combine com PROTON_SONY_HIDRAW_XINPUT=1; controle Xbox usa direto. Com o fallback ativo, DS4 e DualSense/Edge são detectados dinamicamente (inclusive depois de hotplug) e o perfil anunciado se atualiza; para qualquer outro controle, ou se nenhum estiver conectado, o padrão é Xbox.\nVariantes: PROTON_STEAMINPUT_XINPUT_FALLBACK=1 continua valendo como alias de compatibilidade. As variantes PROTON_STEAMINPUT_LAYOUT_XBOX, _DS4 e _DS5 fixam o perfil quando a detecção automática errar.",
				EN: "Provides an XInput-backed replacement for the Steam Input interface some games require. It makes controllers work outside Steam and works around Steam Input profile switching being unavailable under Wine-Wayland.\nWhen it stays inactive: if native Steam Input is enabled for the session, the native controller and mappings stay authoritative.\nPer controller: for Sony controllers, combine it with PROTON_SONY_HIDRAW_XINPUT=1; Xbox controllers use the fallback directly. While active, the fallback detects connected DS4 and DualSense/Edge devices dynamically (including after hotplug) and updates the advertised profile; for anything else, or with nothing connected, it defaults to Xbox.\nVariants: PROTON_STEAMINPUT_XINPUT_FALLBACK=1 is still supported as a compatibility alias. The PROTON_STEAMINPUT_LAYOUT_XBOX, _DS4 and _DS5 variants pin the profile when auto-detection picks wrong.",
			},
		},
		{
		...
		},
		{
			...
			Description: Localized{
				PT: "Liga a camada implícita VK_LAYER_MESA_anti_lag, a implementação open-source da extensão VK_AMD_anti_lag que o Mesa traz desde a 25.3.\nRessalva 1 — a camada é OPCIONAL: precisa estar em /usr/share/vulkan/implicit_layer.d/VkLayer_MESA_anti_lag.json, e vários builds não a incluem (o runtime Flatpak do Mesa, por exemplo), então a variável não faz nada. Confira com vulkaninfo.\nRessalva 2 — ela não é global: só age em jogos que realmente chamam vkAntiLagUpdateAMD, ou seja, que implementam a extensão. Muito menos jogos do que o número de menus de Anti-Lag sugere; nos testes do low_latency_layer ela chegou a parecer no-op.\nDesligue com DISABLE_LAYER_MESA_ANTI_LAG=1. Atenção: o Proton-CachyOS já faz isso sozinho quando PROTON_FSR4_UPGRADE está ativo, porque as duas brigam.\nExiste uma implementação nativa de VK_AMD_anti_lag para o RADV ainda em aberto (MR 42048, não mergeada); quando entrar, essa camada ficará desnecessária. Até lá, prefira LOW_LATENCY_LAYER=1 com LOW_LATENCY_LAYER_REFLEX=1, que tem o mesmo efeito e bem mais jogos.",
				EN: "Enables the implicit VK_LAYER_MESA_anti_lag layer, the open-source implementation of the VK_AMD_anti_lag extension that has shipped with Mesa since 25.3.\nCaveat 1 — the layer is OPTIONAL: it must be present as /usr/share/vulkan/implicit_layer.d/VkLayer_MESA_anti_lag.json, and many builds don't include it (the Mesa Flatpak runtime, for example), so the variable does nothing. Check with vulkaninfo.\nCaveat 2 — it is not global: it only acts on games that actually call vkAntiLagUpdateAMD, i.e. those implementing the extension. Far fewer than the number of Anti-Lag menus suggests; in low_latency_layer's benchmarks it looked like a no-op.\nTurn it off with DISABLE_LAYER_MESA_ANTI_LAG=1. Note that Proton-CachyOS already does that on its own whenever PROTON_FSR4_UPGRADE is active, because the two clash.\nA native VK_AMD_anti_lag implementation for RADV is still an open MR (42048, not merged); once it lands, this layer becomes unnecessary. Until then, prefer LOW_LATENCY_LAYER=1 with LOW_LATENCY_LAYER_REFLEX=1, same effect and far more games.",
			},
		},
		{
		...
		},
		{
			...
			Description: Localized{
				PT: "Faz o low_latency_layer reportar a GPU como NVIDIA para a aplicação, independente do hardware real. Use junto de LOW_LATENCY_LAYER=1 e LOW_LATENCY_LAYER_REFLEX=1. O próprio autor marca como não recomendado e manda preferir DXVK_CONFIG=\"dxgi.hideAmdGpu = True\", porque esta opção é conhecida por quebrar o caminho de upgrade do FSR4 (PROTON_FSR4_UPGRADE) — o mesmo defeito do PROTON_FORCE_NVAPI=1, ainda que por outro mecanismo. Trate como último recurso: também não é à prova de anti-cheat.",
				EN: "Makes low_latency_layer report the GPU as NVIDIA to the application, whatever the real hardware. Use it together with LOW_LATENCY_LAYER=1 and LOW_LATENCY_LAYER_REFLEX=1. The author marks it as not recommended and points to DXVK_CONFIG=\"dxgi.hideAmdGpu = True\" instead, because this option is known to break Proton's FSR4 upgrade path (PROTON_FSR4_UPGRADE) — the same defect as PROTON_FORCE_NVAPI=1, by a different mechanism. Treat it as a last resort: it is not anti-cheat safe either.",
			},
		},
		{
		...
		},
		{
			...
			Description: Localized{
				PT: "Limita o FPS dos jogos D3D12.\nOnde existe: só dentro do fork vkd3d-low-latency — a vkd3d-proton upstream nunca teve essa variável. Não combine sem PROTON_VKD3D_LOWLATENCY=1.\nExceção: o Proton-EM a emula convertendo para a DXVK_CONFIG, e aí ela nem alcança o D3D12, porque quem traduz o D3D12 é a vkd3d, não o DXVK.\nAtenção: o limite está integrado à lógica de frame pacing do fork (FPS limiting is fully integrated into the frame pacing logic), ou seja, não é um corte no fim do pipeline. Sobre a prioridade, o mantenedor é explícito na discussion #2: \"VKD3D_FRAME_RATE takes priority over the Reflex fps cap if both are set\". Ele não diz o mesmo do limite que o jogo impõe por conta própria, então não some os dois esperando um número previsível.\nEm jogos UE4 com swapchain aguardável ele também evita a queda de desempenho do r.OneFrameThreadLag=1 (veja a entrada do PROTON_VKD3D_LOWLATENCY).",
				EN: "Caps FPS in D3D12 games.\nWhere it exists: only inside the vkd3d-low-latency fork — upstream vkd3d-proton never had this variable. Don't combine it without PROTON_VKD3D_LOWLATENCY=1.\nException: Proton-EM emulates it by converting to DXVK_CONFIG, and there it doesn't even reach D3D12, because vkd3d does the D3D12 translation, not DXVK.\nNote: the limit is fully integrated into the fork's frame pacing logic, i.e. it is not a cut at the end of the pipeline. On priority, the maintainer is explicit in discussion #2: \"VKD3D_FRAME_RATE takes priority over the Reflex fps cap if both are set\". He says nothing of the cap the game imposes on its own, so don't stack the two expecting a predictable number.\nIn UE4 games with waitable swapchains it also avoids the r.OneFrameThreadLag=1 performance drop (see the PROTON_VKD3D_LOWLATENCY entry).",
			},
		},
		{
			...
			Description: Localized{
				PT: "Faz o low_latency_layer expor VK_NV_low_latency2 em vez de anti-lag, ativando o Reflex: mesma latência do Anti-Lag 2, mas com suporte em muito mais jogos. É a via preferida — o spoofing de GPU é último recurso e costuma falhar.\nOrdem de tentativa: 1) LOW_LATENCY_LAYER_REFLEX=1 sozinho; 2) acrescente DXVK_CONFIG=\"dxgi.hideAmdGpu = True\" (às vezes também DXVK_NVAPI_ALLOW_OTHER_DRIVERS=1); 3) por último PROTON_FORCE_NVAPI=1 e/ou LOW_LATENCY_LAYER_SPOOF_NVIDIA=1.\nSe o jogo ainda recusar a API: o mantenedor do vkd3d-low-latency sugere forjar o device inteiro por DXVK_CONFIG — dxgi.customVendorId=10de, dxgi.customDeviceId=2206, dxgi.customDeviceDesc=\"NVIDIA GeForce RTX 3080\" (também dá para pôr num dxvk.conf na pasta do jogo).\nAVISOS — quebra o FSR 4: PROTON_FORCE_NVAPI=1 e LOW_LATENCY_LAYER_SPOOF_NVIDIA=1 quebram o upgrade do FSR4 (PROTON_FSR4_UPGRADE). Na issue #2 do projeto isso aparece como o FSR4 sumindo do menu do jogo e ficando inutilizável com a camada ativa. A combinação que a issue dá como funcionando é PROTON_FSR4_UPGRADE=1 com LOW_LATENCY_LAYER_REFLEX=1 e DXVK_CONFIG=\"dxgi.hideAmdGpu = True\" — o spoofing de GPU é o que some. (As versões 4.1.1+ e o custo em RDNA3 aparecem em fóruns, não no upstream: não confirmados aqui.)\nAVISOS — o spoofing não é à prova de falhas: jogos com verificações extras de GPU (THE FINALS tem uma exceção no próprio Proton, Test Drive: Solar Crown checa o device) podem recusar a API mesmo com o menu visível; e em jogos com anti-cheat ele pode ser barrado ou causar kick — não use com anti-cheat.\nComo verificar: procure \"nvapi64:<-NvAPI_D3D_SetSleepMode (Enabled/0us): OK\" no log com PROTON_LOG=1. O menu aparecer não garante que esteja ativo.",
				EN: "Makes low_latency_layer expose VK_NV_low_latency2 instead of anti-lag, enabling Reflex: the same latency as Anti-Lag 2 but supported in far more games. This is the preferred path — GPU spoofing is a last resort and often fails.\nOrder to try: 1) LOW_LATENCY_LAYER_REFLEX=1 alone; 2) add DXVK_CONFIG=\"dxgi.hideAmdGpu = True\" (sometimes DXVK_NVAPI_ALLOW_OTHER_DRIVERS=1 too); 3) only then PROTON_FORCE_NVAPI=1 and/or LOW_LATENCY_LAYER_SPOOF_NVIDIA=1.\nIf the game still refuses the API: the vkd3d-low-latency maintainer suggests forging the whole device via DXVK_CONFIG — dxgi.customVendorId=10de, dxgi.customDeviceId=2206, dxgi.customDeviceDesc=\"NVIDIA GeForce RTX 3080\" (you can also drop these in a dxvk.conf next to the game).\nWARNINGS — it breaks FSR 4: PROTON_FORCE_NVAPI=1 and LOW_LATENCY_LAYER_SPOOF_NVIDIA=1 break the FSR4 upgrade (PROTON_FSR4_UPGRADE). In the project's issue #2 this shows up as FSR4 vanishing from the game's menu and being unusable while the layer is active. The combination the issue reports as working is PROTON_FSR4_UPGRADE=1 with LOW_LATENCY_LAYER_REFLEX=1 and DXVK_CONFIG=\"dxgi.hideAmdGpu = True\" — it is the GPU spoofing that has to go. (The 4.1.1+ versions and the RDNA3 cost come from forum posts, not upstream: not confirmed here.)\nWARNINGS — spoofing isn't foolproof: games with extra GPU checks (THE FINALS has an exception in Proton itself, Test Drive: Solar Crown checks the device) may refuse the API even with the menu visible; and in games with anti-cheat it may be blocked or get you kicked — don't use it with anti-cheat.\nHow to verify: look for \"nvapi64:<-NvAPI_D3D_SetSleepMode (Enabled/0us): OK\" in the PROTON_LOG=1 log. The menu showing up doesn't guarantee it's active.",
			},
		},
		{
		...
		},
		{
			...
			Description: Localized{
				PT: "Habilita o suporte a patentes do NVIDIA Reflex no dxvk-nvapi. Útil quando o menu de Reflex não aparece em jogos que têm suporte.\nO que ele faz de verdade: no script do Proton, PROTON_FORCE_NVAPI define três variáveis de uma vez e sem condição — DXVK_NVAPI_ALLOW_OTHER_DRIVERS=1, DXVK_NVAPI_DRIVER_VERSION=99999 e WINE_HIDE_AMD_GPU=1. É por isso que funciona em GPU AMD (é o que abre o Reflex lá), mas o WINE_HIDE_AMD_GPU impede o jogo de enxergar a AMD e quebra o upgrade FSR 4 (PROTON_FSR4_UPGRADE) — sempre, não às vezes.\nPrefira spoofing direto: o DXVK_NVAPI_ALLOW_OTHER_DRIVERS=1 já vem junto com esta variável, então basta o DXVK_CONFIG=\"dxgi.hideAmdGpu = True\", que expõe o mesmo Reflex sem o WINE_HIDE_AMD_GPU. (A variável fica FORA das aspas: o DXVK_CONFIG separa opções por ponto e vírgula.)\nAtenção ao anti-cheat: forjar o device pode ser barrado. Evite em THE FINALS se possível — o Proton já desliga o NVAPI em GPU não-NVIDIA nesse título, e o PROTON_FORCE_NVAPI é a única forma de furar isso, ao custo do FSR 4.",
				EN: "Enables NVIDIA Reflex patent support in dxvk-nvapi. Useful when the Reflex menu doesn't show up in games that do support it.\nWhat it really does: in the Proton script, PROTON_FORCE_NVAPI sets three variables at once, with no condition — DXVK_NVAPI_ALLOW_OTHER_DRIVERS=1, DXVK_NVAPI_DRIVER_VERSION=99999 and WINE_HIDE_AMD_GPU=1. That is why it works on AMD GPUs (it is what opens Reflex there), but WINE_HIDE_AMD_GPU stops the game from seeing the AMD GPU and breaks the FSR 4 upgrade (PROTON_FSR4_UPGRADE) — always, not sometimes.\nPrefer direct spoofing: DXVK_NVAPI_ALLOW_OTHER_DRIVERS=1 already comes along with this variable, so all you need is DXVK_CONFIG=\"dxgi.hideAmdGpu = True\", which exposes the same Reflex without WINE_HIDE_AMD_GPU. (The variable goes OUTSIDE the quotes: DXVK_CONFIG separates options with a semicolon.)\nWatch out for anti-cheat: forging the device may be blocked. Avoid it in THE FINALS if you can — Proton already disables NVAPI on non-NVIDIA GPUs in that title, and PROTON_FORCE_NVAPI is the only way around that, at the cost of FSR 4.",
			},
		},
		{
		...
		},
		{
			...
			Description: Localized{
				PT: "É a forma suportada de limitar FPS no DXVK atual: o DXVK 3.0 removeu a variável DXVK_FRAME_RATE e deixou só opções de configuração.\nAs três opções: dxgi.maxFrameRate para D3D10/11, d3d9.maxFrameRate para D3D9, e dxvk.maxFrameRate para todos de uma vez (use esta e dispense as outras duas). O default de todas é 0, que é sem limite.\nVale para D3D9/10/11. Em D3D12 quem traduz é a vkd3d, e o truque não pega.\nAo combinar: o DXVK_CONFIG aceita várias opções separadas por ponto e vírgula, mas outra entrada sua que também use DXVK_CONFIG precisa ter o mesmo valor — senão o app avisa que a variável está definida duas vezes com valores diferentes.\nAlternativa preferida pelo próprio DXVK: limitador externo (Gamescope, MangoHud), que costuma dar um resultado mais suave.",
				EN: "This is the supported way to cap FPS on current DXVK: DXVK 3.0 removed the DXVK_FRAME_RATE variable and left only configuration options.\nThe three options: dxgi.maxFrameRate for D3D10/11, d3d9.maxFrameRate for D3D9, and dxvk.maxFrameRate for all of them at once (use that one and skip the other two). The default for all of them is 0, meaning no limit.\nIt covers D3D9/10/11. On D3D12 vkd3d does the translation, so the trick misses.\nWhen combining: DXVK_CONFIG accepts several options separated by semicolons, but another entry of yours that also uses DXVK_CONFIG needs the same value — otherwise the app warns that the variable is set twice with different values.\nAlternative preferred by DXVK itself: an external limiter (Gamescope, MangoHud), which usually gives a smoother result.",
			},
		},
		{
			...
			Description: Localized{
				PT: "Removida no DXVK 3.0: a partir dessa versão a variável não faz nada no DXVK upstream, e o próprio release note manda usar limitador externo (Gamescope, MangoHud) ou a opção de configuração.\nOnde ainda funciona: no fork dxvk-low-latency, sempre junto com PROTON_DXVK_LOWLATENCY=1; e no Proton-EM, por emulação. Sem nenhum dos dois, troque por DXVK_CONFIG=\"dxgi.maxFrameRate=60;d3d9.maxFrameRate=60\" — dxgi.maxFrameRate para D3D10/11, d3d9.maxFrameRate para D3D9, dxvk.maxFrameRate para todos de uma vez.\nNão vale para D3D12: nesses use VKD3D_FRAME_RATE.\nNo fork: em DXVK_FRAME_PACE=low-latency-vrr o próprio DXVK já põe teto de 5% abaixo do refresh, e esse teto pode ser sobrescrito manualmente por aqui mesmo (ex.: 225 em 240 Hz) — o DXVK_HUD=latencydetails mostra o buffer de V-Sync, que é como se acha o valor (release 3.1.1). O upstream não diz o que acontece ao somar um segundo limitador em cima disso: trate a soma como não testada.\nÚtil em telas de 60 Hz, ou para reduzir consumo/ruído quando o jogo passa folgado do refresh.",
				EN: "Removed in DXVK 3.0: as of that version the variable does nothing on upstream DXVK, and the release note itself tells you to use an external limiter (Gamescope, MangoHud) or the configuration option instead.\nWhere it still works: in the dxvk-low-latency fork, always paired with PROTON_DXVK_LOWLATENCY=1; and on Proton-EM, through emulation. With neither, use DXVK_CONFIG=\"dxgi.maxFrameRate=60;d3d9.maxFrameRate=60\" — dxgi.maxFrameRate for D3D10/11, d3d9.maxFrameRate for D3D9, dxvk.maxFrameRate for everything at once.\nIt does not apply to D3D12: use VKD3D_FRAME_RATE there.\nIn the fork: in DXVK_FRAME_PACE=low-latency-vrr mode DXVK itself already caps at 5% below refresh, and that cap can be overridden manually from here (e.g. 225 on a 240 Hz panel) — DXVK_HUD=latencydetails shows the V-Sync buffer, which is how you find the value (release 3.1.1). Upstream says nothing about what happens when a second limiter is stacked on top: treat the combination as untested.\nUseful on 60 Hz displays, or to cut power/noise when the game runs well past refresh.",
			},
		},
		{
		...
		},
		{
			...
			Description: Localized{
				PT: "Escolhe qual perfil do lsfg-vk usar, com o nome entre aspas quando tiver espaço (ex.: LSFGVK_PROFILE='Test Profile' %command%). Este comando usa o perfil \"steam\"; a lista de perfis e os nomes exatos ficam no lsfg-vk-ui.\nForma mais conveniente, segundo a doc oficial: nem é a variável, e sim marcar o executável no campo \"Active In\" do próprio perfil, que aí ativa sozinho em todo lançamento.\nDois avisos que a doc deixa claros: o VSync precisa estar ligado, senão nenhum frame generation acontece (o lsfg-vk liga para você, mas se você desativou, não tem FG); e cuidado com qual .exe você marca, porque jogos UE4/5 costumam ter vários .exe e só um é o certo.\nFerramentas: lsfg-vk-cli healthcheck confere a instalação, lsfg-vk-cli benchmark mede na sua máquina, e DISABLE_LSFGVK=1 desliga tudo. Este comando é específico do perfil \"steam\" — se o seu tem outro nome, troque.\nMigração para a v2.0.0 (setembro de 2026): troque para o branch \"lsfg-vk\" do Lossless Scaling na Steam, senão não é esse build; o layout do arquivo de configuração mudou e é incompatível com a v1, então quem vinha da v1 precisa desinstalar antes (o healthcheck acusa arquivo antigo sobrando); a v2 exige só Vulkan 1.2, o que abre para qualquer GPU Vulkan; e o pipeline bindless novo é 2-3x mais rápido em GPU com FP16 2:1 (AMD laptop e handheld), usando 30% da memória.",
				EN: "Chooses which lsfg-vk profile to use, quoted when the name has spaces (e.g. LSFGVK_PROFILE='Test Profile' %command%). This command uses the \"steam\" profile; the profile list and exact names live in lsfg-vk-ui.\nThe more convenient option, per the official docs: not the variable at all, but marking the executable in the profile's own \"Active In\" field, which then activates by itself on every launch.\nTwo warnings the docs make explicit: VSync must be enabled or you get no frame generation at all (lsfg-vk turns it on for you, but if you disabled it there is no FG); and be careful which .exe you mark, because UE4/5 titles usually ship several and only one is the right one.\nTools: lsfg-vk-cli healthcheck verifies the installation, lsfg-vk-cli benchmark measures on your machine, and DISABLE_LSFGVK=1 turns everything off. This command targets the \"steam\" profile specifically — swap it if yours is named differently.\nMigrating to v2.0.0 (September 2026): switch to the \"lsfg-vk\" branch of Lossless Scaling on Steam, otherwise it isn't that build; the config file layout changed and is incompatible with v1, so anyone coming from v1 has to uninstall first (healthcheck spots a leftover old file); v2 only requires Vulkan 1.2, which opens it up to any Vulkan GPU; and the new bindless pipeline is 2-3x faster on GPUs with FP16 2:1 (AMD laptop and handheld), using 30% of the memory.",
			},
		},
		{
